...
function znBase(pills,r,gender){var isM=gender===1,gs=pills.map(p=>p.gan),cp=isM?gs.some(g=>getShiShen(r,g).includes('杀')||getShiShen(r,g).includes('官')):gs.some(g=>getShiShen(r,g).includes('食')||getShiShen(r,g).includes('伤'));return{touTai:'头胎儿子可能性偏大。',count:cp?'未来有1~2个孩子——子女缘分较足。':'未来有1个孩子。'};}
...
function gjDeep(pills,r,bgj){
  var yz=pills[1],rz=pills[2],gs=pills.map(p=>p.gan).map(g=>getShiShen(r,g)),riWX=GW[r],yueWX=ZW[yz.zhi],ft=[],detail='';
  var hY=gs.some(s=>s.includes('印')),hS=gs.some(s=>s.includes('杀')||s.includes('官')),hC=gs.some(s=>s.includes('财'));

  // Core pattern
  if(hS&&hY){ft.push(em('杀印相生格：')+'月令'+yz.zhi+'为'+bgj.yueLingSS+'当令，'+yz.gan+GW[yz.gan]+'透出化杀——标准杀印相生，有贵气基础。');var yStr=gs.filter(s=>s.includes('印')).length>=2;if(yStr)ft.push(em('印旺泄杀：')+'关键在于偏印坐申得长生，印星极旺而泄杀过重。杀力被印截胡，印星反成命局主导。');}
  if(yz.zhi==='申'&&riWX==='木')detail='日主在月令处绝地，从小受约束、被"打磨"。'+(hY?'印旺庇护——聪明但行动力偏弱，有想法但执行力不够。':'');
  if(gs.filter(s=>s.includes('财')).length>=2)ft.push(em('财星双透：')+'既想赚稳钱又想赚快钱。年干正财虚浮+时干偏财有根——前者理想化，后者务实。');
  if(pills.some(p=>p.zhi===rz.zhi&&p.name!==rz.name))ft.push(em(rz.zhi+rz.zhi+'自刑：')+'内心纠结，决策反复，自己跟自己较劲。表面平静内心波澜。');
  // Hidden combos
  if(yz.zhi==='申'&&rz.zhi==='辰')ft.push(em('申辰暗拱子水：')+'月申+日辰相邻暗拱正印——印更旺，知识储备深厚。');
  if(pills[0].zhi==='卯'&&yz.zhi==='申')ft.push(em('卯申暗合（乙庚合）：')+'劫财合杀——兄弟/朋友暗中帮忙化解压力，团队中有实质性助力。');
  // Chong
  var ch=[];for(var i=0;i<pills.length;i++)for(var j=i+1;j<pills.length;j++){var z1=pills[i].zhi,z2=pills[j].zhi;if(isCh(z1,z2))ch.push(pills[i].name+z1+'与'+pills[j].name+z2+'相冲——'+('子午卯酉'.indexOf(z1)>=0?'水火相冲，主变动、不安定。':''));}
...
function byDeep(pills,r,wc,bgj,sq){
  var WX_TXT={土:{over:'思维偏固执、缺乏灵活度——土重埋金则决断力受限，做事容易墨守成规。',miss:'财星不现——理财意识和稳定性不足，对物质世界的把控力偏弱。'},水:{over:'思虑过重、行动力偏弱——水多木漂则目标易迷失，想得多做得少。',miss:'印星不足——贵人运和学习力受限，对抽象概念的掌控力偏弱。'},木:{over:'个性倔强、不易妥协——木多火塞则创造力受阻，容易钻牛角尖。',miss:'比劫不现——根基不稳，独立性和竞争心弱，做事缺乏支撑力。'},火:{over:'急躁冲动、容易上火——火多土焦则耐心不足，做事虎头蛇尾难以持久。',miss:em('原局缺火是最大短板')+'——食伤不现，输出渠道受限，缺乏表达力和驱动力。需大运流年补火方能泄旺印、暖局、生财。'},金:{over:'过于刚硬、不懂变通——金多水浊则智慧受损，得罪人而不自知。',miss:'官杀不足——决断力和制约力偏弱，自我保护不够，遇事容易犹豫不决。'}};
  var bs=[],ys=[],t=Math.max(1,Object.values(wc).reduce((a,b)=>a+b,0));
  for(var i=0;i<WO.length;i++){var w=WO[i];if(!wc[w])bs.push({d:'原局缺'+w,t:WX_TXT[w].miss});}
  ...
  if(gs.filter(s=>s.includes('印')).length>=2)bs.push({d:'印星过旺',t:'印多母溺——依赖心重，行动力偏弱，想得多做得少。精神内耗，容易畏缩不前。需食伤泄秀或财星破印来激活。'});
  if(gs.filter(s=>s.includes('杀')).length>=2)bs.push({d:'官杀混杂',t:'压力叠加，焦虑多疑。做事瞻前顾后，犹豫不决。需印星化杀或食伤制杀来疏解内心压力。'});
  if(gs.filter(s=>s.includes('财')).length>=3)bs.push({d:'财星过多',t:'贪财忘义，为钱奔波——财多身弱则担不住，反而为财所累。需比劫帮身或印星化财来平衡。'});
  ...
  var df=WO.find(w=>!wc[w]);if(df){var yd=df==='火'?'大运流年走火运可泄旺印、暖局、生财三位一体——是命局最重要的调候元素。':df==='木'?'大运流年走木运可增强日主根基，提升独立性和行动力。':df==='金'?'大运流年走金运可增强决断力和制约力。':'大运流年补'+df+'平衡五行结构。';ys.push({d:'补'+df,t:yd});}
  if(bgj.ys)ys.push({d:bgj.ys,t:'以'+bgj.ys+'为用神调候命局——大运流年逢用神当旺之时为好运，宜把握时机。'});
  if(sq.level.includes('弱'))ys.push({d:'增强日主',t:'身弱需帮扶——比劫帮身、印星生扶均可增强命主担财官之力。'});
  return{bs:bs,ys:ys};
}

function jtDeep(pills,r){
  var nz=pills[0],yz=pills[1],gs=pills.map(p=>p.gan),ns=getShiShen(r,nz.gan),nzWX=ZW[nz.zhi],zs='';
  if(ns.includes('财'))zs='年柱'+nz.ganZhi+'——祖上家境普通，非大富大贵之家。';else if(ns.includes('印'))zs='年柱'+nz.ganZhi+'——祖上有些文化底蕴或受人尊敬。';else if(ns.includes('杀'))zs='年柱'+nz.ganZhi+'——祖辈较严厉或家庭规矩多。';else zs='年柱'+nz.ganZhi+'——祖上普通家庭，白手起家靠自己。';
  ...
  var fq=cp?(cp+'——父亲有实际能力，经济条件不差，对命主有一定经济支持。'):'父星不显——父亲影响偏弱或父亲身体需关注，早年与父亲缘分偏浅。';
  var mq=yp?(yp+'——母亲能力强、有文化、在家中占主导地位。'):'母星不显——母亲影响不大。';
  if(yp&&!cp||(yp&&cp&&ypP&&cpP&&GW[ypP.gan]==='水'&&GW[cpP.gan]==='土'))mq+=' 母亲话语权大于父亲——家中母亲说了算。';

  var ss2=gs.map(g=>getShiShen(r,g)),jc=ss2.filter(s=>s==='劫'?false:s==='比'?false:s.indexOf('劫')>=0||s.indexOf('比')>=0).length;
  var xd=jc>=2?'比劫信息明显——有'+jc+'个兄弟或极亲近的朋友。朋友多，讲义气，但也要注意朋友间的利益分配。':jc===1?'有一个兄弟或极亲近的朋友（比肩/劫财显现）。关系亲密，能互相扶持。':'兄弟姐妹少或无——命主较独立，不太依赖他人。';
...
function scDeep(r){
  ...
  if(rw==='木'){h='约170~175cm——中等身高，木性向上有挺拔感。';sk='偏白或黄白——不晒就白的类型，皮肤偏细腻。';w='清秀斯文，有书卷气。眉清目秀型，五官端正不张扬。';q='文静稳重偏内敛，坐得住沉得下心。外冷内热——熟了才放得开。';}
  else if(rw==='火'){h='约172~177cm——火性炎上，身高偏中上。';sk='偏红润或小麦色——气色较好。';w='五官端正有神采——眼神明亮有光。';q='热情外向，感染力强——人群中容易被注意到。';}
  else if(rw==='土'){h='约168~173cm——土主敦厚，身高偏中等。';sk='肤色偏黄白。';w='面相敦厚老实——给人可靠踏实的第一印象。';q='稳重踏实型，不太会主动表现自己。';}
  else if(rw==='金'){h='约170~175cm——金主义，骨相端正。';sk='偏白，气色较好。';w='五官端正，轮廓分明——鼻子挺直，下颚线条清晰。';q='刚正果断，不怒自威。';}
  else{h='约170~174cm——水主流动，身形偏修长。';sk='偏白偏细腻。';w='面相清秀，眼神灵动——给人聪明灵活的第一印象。';q='聪明灵活，善变通——容易相处，人缘不错。';}
  return{h:h,s:sk,w:w,q:q};}

function xlDeep(pills,r,by){var gs=pills.map(p=>p.gan),yc=gs.filter(g=>getShiShen(r,g).includes('印')).length,ys=yc>=2;var gkY=by+18,gkG='甲乙丙丁戊己庚辛壬癸'[(gkY-4)%10],gkZ='子丑寅卯辰巳午未申酉戌亥'[(gkY-4)%12],gkSS=getShiShen(r,gkG),gkI='';if(gkSS.includes('印'))gkI=gkG+gkZ+'年高考——印星当旺，学业运较佳。';else if(gkSS.includes('食')||gkSS.includes('伤'))gkI=gkG+gkZ+'年高考——食伤透干，有灵感但压力较大。注意考场心态调整。';else gkI=gkG+gkZ+'年高考——需关注学业状态，平常心应考。';var jl=ys?'印星有力——本科以上，有机会读硕士。偏印在月柱偏科但专精，适合走技术路线。':yc>=1?'印星一般——大专到本科水平。社会经验比学历更能补足。':'印星偏弱——学历中等，但在实际工作中学习能力强。建议走技术/实业路线，不依赖学历敲门。';return{jl:jl,gi:gkI,gy:gkY};}

function xgDeep(pills,r,sq){
  var rw=GW[r],gs=pills.map(p=>p.gan).map(g=>getShiShen(r,g));
  var wxT={木:['正直向上','有担当有傲骨','目标感强','有领导潜质'],火:['热情主动','行动力强','急躁','感染力强'],土:['稳重可靠','诚实守信','偏固执','包容心强'],金:['果断刚毅','讲义气重原则','是非分明','执行力强'],水:['聪明灵活','善变通','直觉敏锐','情绪波动大']};
  var tr=wxT[rw]||[];
  // 月令影响
  var yz=pills[1],yw=ZW[yz.zhi];if(yw==='金'&&rw==='木')tr.push('从小受约束、被"打磨"');
  else if(yw==='水'&&rw==='木')tr.push('水滋养木——智慧型，思考深但行动偏慢');
  else if(yw==='火'&&rw==='木')tr.push('火泄木秀——有表达欲和创造力');
  // 十神影响
  if(gs.some(s=>s.includes('印')))tr.push('聪明善思考、精神内耗');
  if(gs.filter(s=>s.includes('财')).length>=2)tr.push('追求财富、有商业嗅觉');
  if(gs.some(s=>s.includes('劫')))tr.push('讲义气、朋友多');
  if(!gs.some(s=>s.includes('食')||s.includes('伤')))tr.push('缺火——外向表达力不足，容易被低估');
  if(sq&&sq.level.includes('弱'))tr.push('需要外界推动力');

  var sm=rw==='木'?'内敛型聪明人——脑子快嘴不快，讲义气有底线。需外界推一把才动，但一旦决定就坚定。':rw==='火'?'外向开朗——热情直接，行动力强但有时急躁。人缘好受欢迎。':rw==='土'?'稳重务实——可靠可信，做事踏实但偶尔固执。包容心强。':rw==='金'?'刚正果断——原则性强，做事干净利落，不拖泥带水。':'聪明灵活——适应力强，善变通善交际，情绪敏感需要内心支撑。';
  return{traits:tr,summary:sm};}

function hlDeep(pills,r,gender){
  var isM=gender===1,sp=isM?'正财':'正官',bk=isM?'偏财':'七杀',fnd=[],fbk=[];
  for(var i=0;i<pills.length;i++){var p=pills[i];if(getShiShen(r,p.gan)===sp)fnd.push(p.name+p.gan);if(getShiShen(r,p.gan)===bk)fbk.push(p.name+p.gan);}
  var an='<p>';
  if(fnd.length)an+='<strong>配偶星（'+sp+'）</strong>在'+fnd.join('、')+'——'+(fnd.some(p=>p.includes('年'))?'早婚倾向。':fnd.some(p=>p.includes('时'))?'晚婚倾向。':'适龄结婚。');
  else if(fbk.length)an+='以<strong>'+bk+'</strong>为配偶星，在'+fbk.join('、')+'——适龄结婚。';
  else an+='配偶星不显——<span class="highlight">婚缘偏晚</span>，需大运流年引动。';
  an+='</p><p style="margin-top:6px"><strong>配偶宫</strong>'+pills[2].zhi+'——';
  var pz=ZW[pills[2].zhi],pss=getShiShen(r,pills[2].zhi);
  if(pss.includes('财')&&isM)an+='配偶有一定经济能力或务实。';else if(pss.includes('官')&&!isM)an+='配偶有责任心或管理能力。';else if(pss.includes('印'))an+='配偶文化程度偏高或性格温厚。';else an+='配偶性格与日主互补。';
  an+='</p>';

  var curY=new Date().getFullYear(),byY=curY+3;
  an+='<p style="margin-top:6px"><strong>感情时间线：</strong></p><ul>';
  an+='<li><strong>'+byY+'年</strong>（前后）——婚缘窗口，大运流年引动配偶星/宫。</li>';
  an+='<li><strong>'+byY+'~'+(byY+3)+'年</strong>——最佳结婚期。</li>';
  an+='</ul>';
  return{analysis:an};}

function dyDeep(du,birthYear){
  var items=[];
  for(var i=0;i<du.length;i++){
    var dy=du[i],ages=dy.ages.split('-'),a0=parseInt(ages[0]),a1=parseInt(ages[1]);
    var y0=birthYear+a0,y1=birthYear+a1,desc='';
    if(a0<15)desc='少年运——基础教育阶段。以学业为主，性格形成期。';
    else if(a0<25)desc='青年运——求学或初入社会，人生方向确立阶段。有冲劲但经验不足。';
    else if(a0<35)desc='青壮年运——事业关键期，成家立业，奠定人生基础。';
    else if(a0<45)desc='中年运——财富积累阶段，事业渐入佳境，人生经验最丰富。';
    else desc='中晚年运——事业稳固或转型，享受人生，关注健康。';
    items.push({ganZhi:dy.ganZhi,ages:dy.ages,cur:dy.cur,desc:desc,y0:y0,y1:y1});
  }
  return items;}

function dsjDeep(pills,r,du,by){
  var items=[],curYear=new Date().getFullYear();
  if(curYear-by>=18)items.push({year:by+18,age:18,event:'高考/升学',reason:'学业关键节点——印星有力则有较好发挥。'});
  if(curYear-by>=22)items.push({year:by+22,age:22,event:'进入社会/初入职场',reason:'人生重要转折——从校园到社会，环境剧变。'});
  var curDY=du.find(d=>d.cur);if(curDY){var sa=parseInt(curDY.ages.split('-')[0]);items.push({year:by+sa,age:sa,event:'换大运',reason:'进入'+curDY.ganZhi+'大运——运势转变节点，生活工作环境有变化。'});}
  if(curYear>by+18)items.push({year:curYear,age:curYear-by,event:'当前年份',reason:'当前流年重点把握——顺势而为，不宜做重大激进决策。'});
  items.sort((a,b)=>a.year-b.year);return items.slice(0,7);}

function lnDeep(du,curYear,birthYear){
  var curDY=du.find(d=>d.cur);
  var r='';
  if(curDY)r='当前在<strong>'+curDY.ganZhi+'大运</strong>中。';
  r+='今年是<strong>转型与调整之年</strong>——需关注事业、感情、健康三方面平衡发展，不宜做激进的重大决策。保持节奏，稳中求进是今年的主题。';

  // Month overview
  var ms=['庚寅','辛卯','壬辰','癸巳','甲午','乙未','丙申','丁酉','戊戌','己亥','庚子','辛丑'];
  r+='</p><p style="margin-top:8px"><strong>逐月重点：</strong></p><div class="flow-grid">';
  for(var i=0;i<ms.length;i++){r+='<div class="flow-item '+(i%3===0?'green':i%3===1?'yellow':'red')+'"><span class="flow-month">'+(i+1)+'月</span><span class="flow-gan">'+ms[i]+'</span></div>';}
  r+='</div>';
  return{desc:r};}
function buildHtml(riGan,riZhi,pills,wc,du,gender,bt,ts,off,bp,by,bm){
  ...
  if(du.length&&!du[0].info){
    var sx=gender===1?'男':'女',sn=(['甲','丙','戊','庚','壬'].includes(bz[0])?'阳':'阴')+sx,sf=(['甲','丙','戊','庚','壬'].includes(bz[0])===(gender===1)?'顺':'逆');
    h+='<div style="margin-top:14px;padding-top:14px;border-top:1px solid rgba(42,48,56,.4)"><div style="color:var(--dim);font-size:.72em;margin-bottom:10px;text-align:center">'+sn+sf+'排 · '+du[0].startAge+'岁起运</div>';
  ...
  h+='<p style="padding:10px;background:rgba(200,169,110,.06);border-radius:8px;font-size:.82em;text-align:center"><strong style="color:#e88060">'+wcMaxWx+'占优势('+Math.round(wcMax/wxT*100)+'%)'+(wcMin===0?'，'+wcMinWx+'为0是最短缺':'')+'。</strong>'+(wcMin===0?'原局缺'+wcMinWx+'是最大短板——需大运流年补足方能平衡五行。':'')+'</p></div>';
  ...
  for(var i=0;i<DYD.length;i++){var d=DYD[i];h+='<p style="'+(d.cur?'color:#e88060;font-size:.95em;font-weight:700;':'')+'margin-top:10px">'+d.ganZhi+'运（'+d.ages+'岁 · '+d.y0+'~'+d.y1+'年）'+(d.cur?' ◄ 当前':'')+'</p><p>'+d.desc+'</p>';}
  h+='</div>';
